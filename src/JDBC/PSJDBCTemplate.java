...
//    }
     ...
     */
    @Override
    public boolean UpdateTime(Timestamp curtime, Timestamp newtime, String rfid)
            throws DataAccessException, CannotGetJdbcConnectionException {
        String SQL = "UPDATE Times SET time = ? WHERE time = ? AND RFID = ?";
        int status = jdbcTemplateObject.update(SQL, newtime, curtime, rfid);
